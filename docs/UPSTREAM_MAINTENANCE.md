...
### 冲突解决铁律（2026-07-26 确立）

**解决任何 UI 文件冲突时，一律以上游版本打底，再把本仓库的样式层贴回去。**

绝对不要反过来——用本仓库的旧版本整体覆盖上游（`git checkout --ours`），那等于丢掉上游在该文件上的全部功能更新与 bug 修复。

判断标准很简单：

| 改动性质 | 处理方式 |
| --- | --- |
| 长什么样（图标、圆角、间距、配色、按钮排列） | 保留本仓库的 |
| 点了做什么、数据怎么走、调用什么 API | 一律用上游的，不要重写 |
| 本仓库新增的独立组件（不与上游争夺同一段逻辑） | 整体保留 |

例：2.1.4 → 2.4.3 那次同步，`ChatInput.kt` 若直接沿用本仓库 3 月的版本，会丢掉 ASR 语音输入、epub 上传、第三方文件选取崩溃修复，以及 `thinkingBudget → reasoningLevel` 的 API 迁移——布局凑对了也编译不过。正确做法是取上游版本，再重新应用图标替换、圆角与间距。
